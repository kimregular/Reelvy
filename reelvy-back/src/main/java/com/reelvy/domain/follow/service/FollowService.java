...
public class FollowService {
    ...
    public List<UserResponse> getFollowersOf(User me) {
        // 나를 구독한 사람 (나는 followed)
        return followRepository.findAllByFollowed(me).stream().map(Follow::getFollower).map(userResponseBuildHandler::buildUserResponseWith).toList();
    }

    public List<UserResponse> getFollowedBy(User me) {
        // 내가 구독한 사람 (나는 follower)
        return followRepository.findAllByFollower(me).stream().map(Follow::getFollowed).map(userResponseBuildHandler::buildUserResponseWith).toList();
    }
}
