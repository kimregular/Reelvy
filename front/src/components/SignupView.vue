...
// 전체 폼 검증
watch(
  [isValidEmailStructure, isDuplicateEmail, isPasswordMatch, isPasswordValidLength, isUsernameValid, email, password],
  () => {
    isInvalidInfo.value =
  ...
  <div class="container mt-5">
    ...
    <form @submit.prevent="handleSignUp">
      ...
      <div class="mb-3">
        ...
        <span v-if="showEmailWarning && !isValidEmailStructure" class="text-danger me-3">이메일 형식에 맞지 않습니다.</span>
        <span v-if="showEmailWarning && !isDuplicateEmail" class="text-primary me-3">사용할 수 있는 이메일입니다.</span>
        <span v-if="showEmailWarning && isDuplicateEmail" class="text-danger">사용할 수 없는 이메일입니다.</span>
        <div class="d-flex align-items-center">
          <input v-model="email" type="email" class="form-control me-2" id="exampleInputEmail1"
                 aria-describedby="emailHelp" style="flex: 4;" required>
          <button type="button" class="btn btn-primary" style="flex: 0.5;" @click="hasDuplicateEmail(email)">Check
          </button>
        </div>
      </div>
      ...
      <div class="mb-3">
        ...
        <span v-if="showUsernameWarning && !isUsernameValid" class="text-danger">사용자 이름은 필수입니다.</span>
        <input v-model="username" type="text" class="form-control" id="username" aria-describedby="emailHelp" required>
      </div>
